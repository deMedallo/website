...
if(
	...
	if($userInfo->id > 0){
		...
		if(strtotime($userInfo->last_activity) < strtotime(date('Y-m-d H:i:s'))){
			#createTX_DM
			$newTX = newTransaccionDM('0x00000000000000000000000000000000', $userInfo->wallets->DM->address, 1);
			
			if(isset($newTX->error) && $newTX->error == false){
				...
				if(isset($update_last_activity->error) && $update_last_activity->error == false){
					...
					}else{ $jsonFinal->data = "0000"; }
				}else{ $jsonFinal->data = "000"; }
